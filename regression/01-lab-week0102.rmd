...
mlb11 %>% ggplot(aes(homeruns,runs))+geom_point()
...
lm(runs ~ homeruns, data = mlb11) %>% summary
...
<li> The residuals are fairly symmetric, with only a slightly longer tail on the right, hence it would be appropriate to deem the the normal distribution of residuals condition met. </li> <- essa
</ol>
...
<li> slugging percentage (`new_slug`) </li>  <- essa aqui
...
```{r diag}
# type your code for the Exercise here, and Knit
lm(runs ~ new_obs, data = mlb11) %>% summary
```
