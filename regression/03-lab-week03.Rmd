...
evals %>% ggplot(aes(score)) +geom_histogram()
evals %>% ggplot(aes(score)) +geom_boxplot()
median(evals$score)

evals %>%select(score) %>%  filter(score <3) %>% length()
```
...
```{r summary-model}
# type your code for the Exercise here, and Knit
score_m <- lm(score ~ bty_avg, data = evals)
summary(score_m)
```
...
plot(score_m$residuals)

hist(score_m$residuals)
qqnorm(score_m$residuals)
qqline(score_m$residuals)
...
mm <- lm(score ~ ethnicity + gender + language + age + cls_perc_eval 
             + cls_students + cls_level + cls_profs + cls_credits + bty_avg, data = evals)
summary(mm)$adj.r.squared
```

10. Elimination of which variable from the full model yielded the highest adjusted R-squared? 
<ol>
<li> `bty_avg` </li> 11
<li> `cls_profs` </li> * 1430
<li> `cls_students` </li> 1401
<li> `rank` </li>
</ol>
